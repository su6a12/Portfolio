$(document).ready(function() {
	...
	$.getJSON("https://raw.githubusercontent.com/FreeCodeCamp/ProjectReferenceData/master/GDP-data.json", function(data) {
		...
		var stats = data.data, year = [], values = [];
		...
		var y = d3.scale.linear().domain([0, d3.max(stats, function(data) { return data[1]; })]).range([innerHeight, 0]);
		...
		var bars = gdpChart.selectAll("rect.bar")
						...
						.append("rect")
						.attr("class", "bar")
						.attr("x", function(each, index) { return x(index) + barWidth; })
						.attr("y", function(each) { return y(each[1]); })
						.attr("height", function(each) { return innerHeight - y(each[1]); })
						.attr("width", barWidth)
						.attr("fill", "#11a9c0")
