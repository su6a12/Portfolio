$(document).ready(function() {
	...
	$.getJSON("https://raw.githubusercontent.com/FreeCodeCamp/ProjectReferenceData/master/cyclist-data.json", function(data) {
		...
		var minTime = d3.min(data, function(each) { return each.Seconds; });
		var maxTime = d3.max(data, function(each) { return each.Seconds; });
		...
		var nameLabels = scatterPlot.selectAll("text.names")
						...
						.text(function(each) { return each.Name; });
		...
		var circles = scatterPlot.selectAll("circle.circle")
						...
						.append("svg:circle")
						.attr("class", "circle")
						.attr("cx", function(each) { return x(each.Seconds - minTime); })
						.attr("cy", function(each) { return y(each.Place); })
						.attr("r", 5)
